  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, errorType) {
      const label = field.closest('.form-group, .field-wrapper, label')
        ? (field.closest('.form-group, .field-wrapper')
            ...
            : field.placeholder || field.name || 'This field')
        : field.placeholder || field.name || 'This field';
      ...
      return messages[errorType] || 'Please check this field.';
    }
    ...
    function validateField(field) {
      ...
      if (isRequired) {
        ...
        if (fieldType !== 'checkbox' && !validators.required(value)) {
          showError(field, getErrorMessage(field, 'required'));
          return false;
        }
      }

      if (value && fieldType === 'email' && !validators.email(value)) {
        showError(field, getErrorMessage(field, 'email'));
        return false;
      }

      if (value && fieldType === 'tel' && !validators.tel(value)) {
        showError(field, getErrorMessage(field, 'tel'));
        return false;
      }

      if (value && minLength && !validators.minlength(value, minLength)) {
  ...
  (function applyRTLSupport() {
    ...
    if (isRTL) {
      ...
      rtlStyle.textContent = [
        '.is-rtl .animate, .is-rtl .fade-in, .is-rtl .fade-in-up,',
        '.is-rtl .scroll-animate, .is-rtl .reveal, .is-rtl [data-animate] {',
        '  transform: translateY(24px);',
        '}',
        '.is-rtl .animate.is-visible, .is-rtl .fade-in.is-visible,',
        '.is-rtl .fade-in-up.is-visible, .is-rtl .scroll-animate.is-visible,',
        '.is-rtl .reveal.is-visible, .is-rtl [data-animate].is-visible {',
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '528cf47d-356f-4200-bac2-a3326096cde5',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '528cf47d-356f-4200-bac2-a3326096cde5';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '528cf47d-356f-4200-bac2-a3326096cde5';
